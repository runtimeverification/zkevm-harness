...
#[unsafe(no_mangle)]
pub static mut PC: usize = 16;

fn main() {
    // Given

    // assume PC <= MAX_CODE_SIZE
    if unsafe { PC } > MAX_CODE_SIZE {
        return;
    }

    let input = Bytes::new();
    let bytecode_iter = unsafe { CODE[..PC].iter().chain(std::iter::once(&OPCODE)) };
