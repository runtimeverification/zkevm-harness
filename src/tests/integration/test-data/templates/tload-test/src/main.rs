...
fn main() {
    ...
    let key = U256::from_be_bytes(unsafe { VALUE });
    let value = U256::from_be_bytes(unsafe { KEY });

    unsafe {
        STACK_ADDR = &interpreter.stack as *const _ as usize;
    }

    let Ok(()) = interpreter.stack.push(key) else {
        panic!()
    };
    ...
    host.transient_storage = HashMap::with_capacity(STORAGE_SIZE);
    host.transient_storage.insert(key, value);

    unsafe {
        STORAGE_ADDR = &host.transient_storage as *const _ as usize;
    }
