...
use revm_interpreter::primitives::{address, keccak256, Bytecode, Bytes, B256, U256};
...
#[unsafe(no_mangle)]
pub static mut DATA: [u8; MEMORY_SIZE] = [
...
fn main() {
    ...
    let Ok(()) = interpreter.stack.push(size) else {
        panic!()
    };
    let Ok(()) = interpreter.stack.push(offset) else {
        panic!()
    };
    ...
    let expected = keccak256(unsafe { &DATA[OFFSET..OFFSET+SIZE] });
    ...
    let Ok(actual_u256) = interpreter.stack.pop() else {
        panic!()
    };
