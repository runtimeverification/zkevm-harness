...
fn main() {
    ...
    unsafe {
        let Ok(()) = interpreter.stack.push(U256::from_be_bytes(VALUE)) else {
            panic!()
        };
    ...
    let memory = SharedMemory::with_capacity(512);
    ...
    let res = interpreter.take_memory().get_u256(usize::from(unsafe { OFFSET }));
    unsafe {
